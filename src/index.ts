...
type StatusFilter = "all" | "active" | "pinned" | "archived" | "trashed";
...
function must<T>(value: T | null, selector: string): T {
  if (!value) throw new Error(`Required DOM element missing: ${selector}`);
  return value;
}
...
const dueDateInput = must(document.querySelector<HTMLInputElement>("#dueDateInput"), "#dueDateInput");
const dueTimeInput = must(document.querySelector<HTMLInputElement>("#dueTimeInput"), "#dueTimeInput");
const reminderOffsetInput = must(
  document.querySelector<HTMLSelectElement>("#reminderOffsetInput"),
  "#reminderOffsetInput"
);
const recurrenceInput = must(document.querySelector<HTMLSelectElement>("#recurrenceInput"), "#recurrenceInput");
...
const statsPanel = must(document.querySelector<HTMLElement>("#statsPanel"), "#statsPanel");
const bulkActions = must(document.querySelector<HTMLElement>("#bulkActions"), "#bulkActions");
...
const undoButton = must(document.querySelector<HTMLButtonElement>("#undoButton"), "#undoButton");
const redoButton = must(document.querySelector<HTMLButtonElement>("#redoButton"), "#redoButton");
const exportButton = must(document.querySelector<HTMLButtonElement>("#exportButton"), "#exportButton");
const importButton = must(document.querySelector<HTMLButtonElement>("#importButton"), "#importButton");
const importFileInput = must(document.querySelector<HTMLInputElement>("#importFileInput"), "#importFileInput");
...
function createNote(
  input: Pick<Note, "title" | "content" | "color" | "tags" | "checklist" | "dueAt" | "remindAt" | "recurrence">
): void {
  runMutation(() => {
    const now = Date.now();
    state.notes.push({
...
function updateNote(
  ...
  patch: Partial<
    Pick<
      Note,
      "title" | "content" | "color" | "tags" | "checklist" | "dueAt" | "remindAt" | "reminderNotifiedAt" | "recurrence"
    >
  >
): void {
  runMutation(() => {
    ...
    if (patch.dueAt !== undefined) note.dueAt = patch.dueAt;
    if (patch.remindAt !== undefined) note.remindAt = patch.remindAt;
    if (patch.reminderNotifiedAt !== undefined) note.reminderNotifiedAt = patch.reminderNotifiedAt;
    if (patch.recurrence !== undefined) note.recurrence = patch.recurrence;
...
function purgeNote(id: string): void {
  runMutation(() => {
    const before = state.notes.length;
    state.notes = state.notes.filter((item) => item.id !== id);
    if (before === state.notes.length) throw new Error("Note not found for purge.");
  });
}
...
function snoozeReminder(noteId: string, minutes: number): void {
  runMutation(() => {
    const note = findNoteOrThrow(noteId, "reminder snooze");
    if (note.remindAt === null) throw new Error("Reminder is not configured for this note.");
...
function applyBulk(action: "archive" | "trash" | "restore" | "purge"): void {
...
function runMutation(mutator: () => void): void {
...
function findNoteOrThrow(id: string, operation: string): Note {
  const note = state.notes.find((item) => item.id === id);
  if (!note) throw new Error(`Note not found for ${operation}.`);
  return note;
}
...
function render(): void {
  ...
  notesContainer.innerHTML = notes
    .map((note) => {
      ...
      const snoozeActions =
        note.trashedAt === null && note.remindAt !== null
          ? `
...
function bindCardActions(): void {
  notesContainer.querySelectorAll<HTMLButtonElement>("button[data-action]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (action === "snooze") {
        ...
        if (!Number.isFinite(minutes)) throw new Error("Invalid snooze minute value.");
  ...
  notesContainer.querySelectorAll<HTMLInputElement>("input[data-action='check']").forEach((box) => {
  ...
  notesContainer.querySelectorAll<HTMLInputElement>("input[data-action='select-note']").forEach((box) => {
    box.addEventListener("change", () => {
      const noteId = box.dataset.id;
      if (!noteId) return;
      if (box.checked) state.selectedIds.add(noteId);
...
function renderBulkActions(): void {
  ...
  bulkActions.querySelectorAll<HTMLButtonElement>("button[data-bulk-action]").forEach((button) => {
    button.addEventListener("click", () => {
...
function loadNotes(): Note[] {
  ...
  if (!Array.isArray(parsed)) throw new Error("Stored notes data is invalid.");

  return parsed.map((item) => normalizeNoteRecord(item));
}

function normalizeNoteRecord(value: unknown): Note {
  if (!value || typeof value !== "object") throw new Error("Stored note item shape is invalid.");
...
function parseDueAtInput(dateValue: string, timeValue: string): number | null {
  ...
  if (Number.isNaN(timestamp)) throw new Error("Invalid due date value.");
  return timestamp;
}

function parseReminderOffsetMinutes(value: string): number | null {
  if (value === "none") return null;
  const minutes = Number.parseInt(value, 10);
  if (!Number.isFinite(minutes) || minutes < 0) throw new Error("Invalid reminder offset.");
  return minutes;
}

function parseRecurrenceMode(value: string): RecurrenceMode {
  if (value === "daily" || value === "weekly") return value;
  return "none";
}

function deriveReminderTimes(
  dueAt: number | null,
  reminderOffsetMinutes: number | null
): { dueAt: number | null; remindAt: number | null } {
  if (dueAt === null || reminderOffsetMinutes === null) return { dueAt, remindAt: null };
  return {
    dueAt,
...
function toDueInputParts(timestamp: number | null): { date: string; time: string } {
...
function inferReminderOffset(dueAt: number | null, remindAt: number | null): string {
  if (dueAt === null || remindAt === null) return "none";
  const diffMinutes = Math.round((dueAt - remindAt) / 60_000);
  if ([0, 5, 15, 60, 1440].includes(diffMinutes)) return String(diffMinutes);
  return "none";
}

function getDueMeta(note: Note): { text: string; overdue: boolean } | null {
  if (note.dueAt === null) return null;
...
function formatReminderLead(dueAt: number, remindAt: number): string {
  const diffMinutes = Math.max(0, Math.round((dueAt - remindAt) / 60_000));
  if (diffMinutes === 0) return "at due time";
...
function runAutoMaintenance(): void {
  let changed = false;
  if (checkDueReminders()) changed = true;
  if (autoCleanupTrash()) changed = true;
  if (changed) persistNotes();
  render();
}

function checkDueReminders(): boolean {
  const now = Date.now();
  let changed = false;

  state.notes.forEach((note) => {
    if (note.trashedAt !== null) return;
    if (note.remindAt === null || note.reminderNotifiedAt !== null) return;
    if (note.remindAt > now) return;
...
function autoCleanupTrash(): boolean {
...
function computeNextDueAt(currentDueAt: number | null, recurrence: RecurrenceMode): number {
...
function pruneSelection(): void {
  ...
  state.selectedIds.forEach((id) => {
    if (!availableIds.has(id)) state.selectedIds.delete(id);
  });
}
...
function importData(rawText: string): void {
  const parsed = JSON.parse(rawText) as unknown;
  if (!parsed || typeof parsed !== "object") throw new Error("Import file must be a JSON object.");
  const payload = parsed as { notes?: unknown; prefs?: unknown };
  if (!Array.isArray(payload.notes)) throw new Error("Import file must contain a notes array.");
  ...
  if (!confirm("Import will replace current notes. Continue?")) return;
...
function normalizeImportedPrefs(value: unknown): Partial<Pick<AppState, "theme" | "layout">> {
  if (!value || typeof value !== "object") return {};
  const prefs = value as { theme?: unknown; layout?: unknown };
  return {
...
function showReminder(note: Note): void {
